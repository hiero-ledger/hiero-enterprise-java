...
public record TokenCreateRequest(
  ...
  public TokenCreateRequest {
    ...
    if (symbol.length() > MAX_SYMBOL_LENGTH) {
      throw new IllegalArgumentException(
          "Symbol length must be less than or equal to " + MAX_SYMBOL_LENGTH);
    }
  }

  public static TokenCreateRequest of(
      @NonNull final String name, @NonNull final String symbol, @NonNull final Account account) {
    return of(
        name,
        symbol,
        account.accountId(),
  ...
  public static TokenCreateRequest of(
    ...
    return of(
        name, symbol, account.accountId(), account.privateKey(), tokenType, account.privateKey());
  }
